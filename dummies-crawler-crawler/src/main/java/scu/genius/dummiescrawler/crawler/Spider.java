package scu.genius.dummiescrawler.crawler;
...
import scu.genius.dummiescrawler.core.concurrent.*;
...
import scu.genius.dummiescrawler.core.listener.SpiderListener;
...
import scu.genius.dummiescrawler.crawler.executor.shape.LoopExecutor;
import scu.genius.dummiescrawler.crawler.utils.ExecutorsUtils;
...
public class Spider {
    ...
    public List<SpiderOutput> run(CrawlerFlow crawlerFlow, SpiderContext context, Map<String, Object> variables) {
        if (variables == null) {
        ...
        // 流程结束通知
        CrawlerTask crawlerTask = crawlerTaskMapper.selectById(context.get("taskId"));
    ...
    public List<SpiderOutput> run(CrawlerFlow crawlerFlow, SpiderContext context) {
        return run(crawlerFlow, context, new HashMap<>());
    }
    ...
    private void executeRoot(SpiderNode root, SpiderContext context, Map<String, Object> variables) {
        ...
        SpiderFlowThreadPoolExecutor.SubThreadPoolExecutor pool = executorInstance.createSubThreadPoolExecutor(Math.max(nThreads, 1) + 1, submitStrategy);
                        ...
                        if (max.isPresent()) {    //判断任务是否完成
                            ...
                            if (context.isRunning()) {    //检测是否运行中(当在页面中点击"停止"时,此值为false,其余为true)
                                ...
                                if (task.executor.allowExecuteNext(task.node, context, task.variables)) {    //判断是否允许执行下一级
                                    log.debug("执行节点[{}:{}]完毕", task.node.getNodeName(), task.node.getNodeId());
                                    //执行下一级
                                    Spider.this.executeNextNodes(task.node, context, task.variables);
                                } else {
                                    log.debug("执行节点[{}:{}]完毕，忽略执行下一节点", task.node.getNodeName(), task.node.getNodeId());
                                }
                            }
                        }
                        //睡眠1ms,让出cpu
    ...
    public void executeNode(SpiderNode fromNode, SpiderNode node, SpiderContext context, Map<String, Object> variables) {
        ...
        log.debug("执行节点[{}:{}]", node.getNodeName(), node.getNodeId());
        //找到对应的执行器
        ShapeExecutor executor = ExecutorsUtils.get(shape);
        if (executor == null) {
            log.error("执行失败,找不到对应的执行器:{}", shape);
        ...
        if (isLoop = StringUtils.isNotBlank(loopCountStr)) {
            try {
                ...
                log.info("获取循环次数{}={}", loopCountStr, loopCount);
            } catch (Throwable t) {
                loopCount = 0;
                log.error("获取循环次数失败,异常信息：{}", t);
            }
        }
        if (loopCount > 0) {
            //获取循环下标的变量名称
                    ...
                    tasks.add(new SpiderTask(TtlRunnable.get(() -> {
                        if (context.isRunning()) {
                            try {
                                ...
                                nVariables.put("ex", t);
                                log.error("执行节点[{}:{}]出错,异常信息：{}", node.getNodeName(), node.getNodeId(), t);
                            }
                        }
